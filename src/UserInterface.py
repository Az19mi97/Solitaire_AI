import os
from action.FindImage import *
import random
from action.CardHandler import *
...
class GameUI:
    ...
    def GetSlicedCardsFromRegion(self, x1, y1, x2, y2, y3, caller = ''):
        ...
        for crd,crdpath in self.gv.cardsInParts.items():
            #print(crd + ":" + crdpath)
            crdNumber = int(crd[1:])
            if crdNumber > 10:
                ...
                if indOfLastRenderedCard != -1 and crdNumber > indOfLastRenderedCard+1:
                    ...
                    while cardFound == False and tryCount < 3:
                        ...
                        for ch in ['s','c','d','h']:
                            ...
                            crdCandidatePath = self.gv.cardsInParts[crdToFind]
                            if crdNumber > 10:
    ...
    def GetNewCardsInColumn(self, x1, y1, caller = ''):
        ...
        absolute_path = os.path.join(os.getcwd(), self.gv.imageDepository, 'parts', 'NewCardBorder.png')
    ...
    def findImgAndClick(self,imageName, precision = 0.90):
        absolute_path = os.path.join(os.getcwd(), self.gv.imageDepository, imageName)
    ...
    def PressSolveBtn(self):
        ...
        absolute_path = os.path.join(os.getcwd(), self.gv.imageDepository, 'SolveBtn.png')
    ...
    def UpdateGameState(self):
        ...
        if "draw_deck" in self.gs.ui_components_to_render:
            ...
            cardsRendered = self.GetCardsFromRegion(self.gv.drawCardArea[0], self.gv.drawCardArea[1],
                                                    self.gv.drawCardArea[2], self.gv.drawCardArea[3], 0.95, "Capture Draw Deck", True)
        ...
        if "top_deck" in self.gs.ui_components_to_render:
            ...
            cardsRendered = self.GetCardsFromRegion(self.gv.cardArea[0], self.gv.cardArea[1], self.gv.cardArea[2], self.gv.cardArea[3], 0.95, "Capture Deck")
        ...
        if "columns" in self.gs.ui_components_to_render:
            ...
            for i in range(0,7):
                if (i+1) not in self.gs.ui_components_to_render['columns']:
                    ...
                    cardsRendered = self.GetCardsFromRegion(left, 325, left + self.gv.widthOfColumn, 1050, 0.95, "Capture Column Cards", True)
                    crdToAdd = []
                    if len(cardsRendered) > 0:
                    ...
                    if len(crdToAdd) > 0:        
                        #print("crdtoadd:")
                        #print(crdToAdd)                      
                        slicedCardsRendered = self.GetSlicedCardsFromRegion(left, 325, left + self.gv.widthOfColumn, 1050, crdToAdd[2] - 25, "Capture All Column Cards")
                        if len(slicedCardsRendered) > 0 and slicedCardsRendered[0][0] != crdToAdd[0]:
                            slicedCardsRendered.insert(0,crdToAdd)
                    else:
                        slicedCardsRendered = self.GetSlicedCardsFromRegion(left, 325, left + self.gv.widthOfColumn, 1050, -1, "Capture All Column Cards")
